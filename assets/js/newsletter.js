(() => {
  ...
  const setIdle = () => {
    btn.disabled = false;
    btn.textContent = btn.dataset.idleText || "Suscribirme";
    msg.textContent = "";
  ...
  const setSending = () => {
    btn.disabled = true;
    btn.textContent = btn.dataset.sendingText || "Enviando…";
    msg.textContent = msg.dataset.sendingText || "Procesando suscripción…";
  };
  ...
  const setSuccess = (email, already) => {
    ...
    btn.textContent = already
      ? btn.dataset.alreadyText || "Ya suscripto ✓"
      : btn.dataset.successText || "Suscripto ✓";
    ...
    const fallback = already
      ? `Hecho: <strong>${safeEmail}</strong> ya estaba suscripto.`
      : `Listo: <strong>${safeEmail}</strong> quedó suscripto.`;
  ...
  const setError = (text) => {
    btn.disabled = false;
    btn.textContent = btn.dataset.retryText || "Reintentar";
    msg.textContent =
      text || msg.dataset.errorText || "Hubo un problema. Probá nuevamente.";
  };

  // Estado inicial: SIEMPRE limpio al cargar (sin persistencia)
  setIdle();

  form.addEventListener("submit", async (e) => {
    e.preventDefault();

    // evita doble submit si spamean click
    ...
    try {
      ...
      const message = data.message || "No se pudo suscribir.";
      const details = typeof data.details === "string" ? data.details : "";
      setError(details ? `${message} (${details.slice(0, 140)})` : message);
    } catch {
      setError("Error de red. Revisá tu conexión y reintentá.");
    }
  });

  // Debug rápido (console): window.raven3NewsletterReset()
